from ai_service.prompt.domain.model.prompt_template import (
    PromptTemplate,
    PromptTemplateProps,
)
...
class GetActivePromptUseCase:
    def __init__(self, repo: IPromptTemplateRepository) -> None:
        self._repo = repo

    async def execute(self, name: str, user_id: str | None = None) -> PromptTemplate:
        if user_id:
